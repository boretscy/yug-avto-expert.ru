            ...
            <div class="col text-center">
                <?php foreach ( $pagination['items'] as $item ) { ?>
                    <?php if ( (int)$item['text'] ) { ?>
                        <span class="c-yadarkgray me-3">
                            <a 
                                href="<?= $app->makeFilterUrl($filter, ['page'=>$item['page']], $item['page']);?>" 
                                class="text-decoration-none <?= (($item['current'])?'c-yayellow c-h-yayellow':'c-yadarkgray c-h-yadarkgray');?>"
                                data-page="<?= $item['page'];?>"
                                role="pagination"
                                ><?= $item['text'];?></a>
                        </span>
                    <?php } else { ?>
                        <span class="c-yadarkgray me-3">
                            <?php if ($item['page']) { ?>
                            <a href="<?= $app->makeFilterUrl($filter, ['page'=>$item['page']], $item['page']);?>" class="c-yadarkgray c-h-yadarkgray text-decoration-none"><?= $item['text'];?></a>
                            <?php } else { ?>
                            <?= $item['text'];?>
                            <?php } ?>
                        </span>
