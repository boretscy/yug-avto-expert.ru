    ...
    class YAppShowroom {
		...
		public function makeFilterBreadcrumbs( $filter, $f ) {
			...
			if ( !empty($filter['city']) && !is_array($filter['city']) ) {
				$res[] = $city = [
					'link' => $this->Conf['baseUrl'].'/'.$this->getCityAlias($filter['city']).'/',
					'text' => $filter['city']
				];
			}
		...
		public function setCityCookie() {
			...
			if ( !empty($c) ) {
				...
				foreach ( explode(',', 'Краснодар,Яблоновский') as $q ) $res[] = $this->getCityAlias($q);
		...
		public function getCityCookie() {
			...
			if ( empty($res) ) foreach ( explode(',', 'Краснодар,Яблоновский') as $q ) $res[] = $q;
			return $res;
		}
		...
		public function getTagName( $items, $code ) {

			foreach ( $items as $item ) if ( $item['code'] == $code ) return $item['name'];
			return ''; 
		}
