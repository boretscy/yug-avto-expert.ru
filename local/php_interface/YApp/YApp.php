<?php
	...
	class YApp extends YAppConfig {
		...
		const IBLOCK_FORMS = 3;

		const IBLOCK_SEO = 17;
		const IBLOCK_VACANCIES = 14;

		const SALT = 'yug-_пошёл_avto-_на хуй_expert_спамер_.ru';
		...
		public static function getCISMeta() {
			...
			$url = 'https://apps.yug-avto.ru/API/get/cis/meta/'.$query['entity'].'/?'.http_build_query($query);
		...
		public static function makeFilterUrl( $GET = [], $params = [] ) {
		...
		public static function setCityCookie() {
			...
			if ( in_array('Новороссийск', $c) || in_array('Майкоп', $c) ) {
				setcookie('SELECTED_CITY', '', time()-3600*24*14, '/');
				$c = [];
			}
			$c = array_unique($c);
			if ( !empty($c) ) {
				if ( 
					...
					foreach ( explode(',', 'Краснодар,Яблоновский') as $q ) $res[] = $q;
					setcookie('SELECTED_CITY', json_encode(array_unique($res)), time()+3600*24*14, '/');
					return 'Краснодар,Яблоновский';
				} else {
					return implode(',', $c);
				}
			} else {
		...
		public static function setFlagCookie() {

			$key = md5( $_SERVER['HTTP_X_REAL_IP'].md5(static::SALT).$_SERVER['HTTP_USER_AGENT'] );

            $log = json_decode( file_get_contents($_SERVER['DOCUMENT_ROOT'].'/logs/check_cookie.json'), true );
			$log[$key] = time();

			file_put_contents($_SERVER['DOCUMENT_ROOT'].'/logs/check_cookie.json', json_encode($log));
			setcookie('YAPPS_ID', $key, ['path' => '/']);

			return false;
		}

		public static function getFlagCookie( $key ) {

			$val = (int)json_decode( file_get_contents($_SERVER['DOCUMENT_ROOT'].'/logs/check_cookie.json'), true )[$key];
			if ( !$val ) return false;
			return ( time() - $val <= 60*30 ) ? true : false;
		}
		...
		public static function getCache( $section, $name, $time = 3600 ) {

			$cache = json_decode(
				file_get_contents(
					__DIR__.'/../../cache/'.$section.'/'.$name.'.json'
				),
				true
			);
			if ( time() <= $cache['time']+$time ) return $cache['data'];
			return false;
		}
		public static function setCache( $section, $name, $data = [] ) {
